...
public class StatisticsWindow extends Stage {
    ...
    public Parent showElement() {
        ...
        if (option == 0) {
            ...
            for (int i = 0; i < game.getProducts().getDishes().size(); i++) {
                gp.add(new Text(game.getProducts().getDishes().get(i).getName()), 0, i + 2);
                gp.add(new Text(String.format("%.2f", game.getPlayers()[0].getStatistics().getSalesYesterday(i, game.getDay())) + " €"), 1, i + 2);
                gp.add(new Text(String.format("%.2f", game.getPlayers()[0].getStatistics().getSalesWeek(i, game.getDay())) + " €"), 2, i + 2);
                gp.add(new Text(String.format("%.2f", game.getPlayers()[0].getStatistics().getSalesAllTime(i, game.getDay())) + " €"), 3, i + 2);
            }
            gp.add(new Text("Gesamt"), 0, gp.getRowCount());
            gp.add(new Text(String.format("%.2f", game.getPlayers()[0].getStatistics().getSalesYesterdayTotal(game.getDay())) + " €"), 1, gp.getRowCount() - 1);
            gp.add(new Text(String.format("%.2f", game.getPlayers()[0].getStatistics().getSalesWeekTotal(game.getDay())) + " €"), 2, gp.getRowCount() - 1);
            gp.add(new Text(String.format("%.2f", game.getPlayers()[0].getStatistics().getSalesAllTimeTotal(game.getDay())) + " €"), 3, gp.getRowCount() - 1);
        }
        ...
        RowConstraints rc = new RowConstraints(27.0);
